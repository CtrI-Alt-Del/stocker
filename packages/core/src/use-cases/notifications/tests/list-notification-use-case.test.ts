...
describe('List notification use case', () => {
  it('should list all stock level notifications', async () => {
    ...
    const fakeStockLevelNotifications = StockLevelNotificationsFaker.fakeMany(20, { companyId });
    ...
    expect(stockNotifications).toEqual(fakeStockLevelNotifications.map((notification) => notification.dto));
  });
  ...
  it('should list all expiration date notifications', async () => {
    ...
    const companyId = 'fixed-company-id';
    const fakeExpirationDateNotifications = ExpirationDateNotificationsFaker.fakeMany(20, { companyId });
