import { ExpirationDateNotification, StockLevelNotification } from '../../../src/domain/entities';
...
export class NotificationsRepositoryMock implements INotificationsRepository {
  ...
  async findManyStockLevelNotifications(companyId: string): Promise<StockLevelNotification[]> {
    return this.notifications.filter(
      (notification): notification is StockLevelNotification =>
        notification instanceof StockLevelNotification && notification.companyId === companyId,
    );
  }
  ...
  async findManyExpirationDateNotifications(companyId: string): Promise<ExpirationDateNotification[]> {
    return this.notifications.filter(
      (notification): notification is ExpirationDateNotification =>
        notification instanceof ExpirationDateNotification && notification.companyId === companyId,
    );
  }
  ...
  async addExpirationDateNotification(expirationNotification: ExpirationDateNotification): Promise<void> {
